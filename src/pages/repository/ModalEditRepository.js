import {
    Box,
    Button,
    FormControl,
    Grid,
    MenuItem,
    Select,
    Table,
    TableBody,
    TableCell,
    TableContainer,
    TableHead,
    TableRow,
    TextField,
} from "@mui/material";
import React, {useEffect, useState} from "react";
import Dialog from "@mui/material/Dialog";
import DialogTitle from "@mui/material/DialogTitle";
import IconButton from "@mui/material/IconButton";
import CloseIcon from "@mui/icons-material/Close";
import {Form, Formik} from "formik";
import {toast} from "react-toastify";
import * as yup from 'yup';
import {TreeSelect} from "primereact/treeselect";
import {sortTreeData} from "../../constants/utils";
import apiRepository from "../../api/repository";
import ClearIcon from '@mui/icons-material/Clear';

export default function ModalEditRepository(props) {
    const {currentRepository,setSelectedNodeKey,listOrganization, isUpdate, openModalEdit, handleCloseModalEdit, isRefresh,setIsRefresh,listOrganizationTree} = props;
    const [disable, setDisable] = useState(false);
    const [expandedKeys, setExpandedKeys] = useState({0: true});
    const [selectedNodeKeysOrganization, setSelectedNodeKeysOrganization] = useState(null);
    const [expandedKeysOrganization, setExpandedKeysOrganization] = useState({0: true});

    const [listOrganizationSelected,setListOrganizationSelected] = useState([
    ])
    const [info, setInfo] = useState({
        id:null,
        name: '',
        code: '',
        description: '',
        parentId: null,
        location:'',
        status:'empty'
    })
    const validationSchema = yup.object({
        name: yup
            .string()
            .trim()
            .required('Không được để trống')
            .max(255, 'Tối đa 255 ký tự'),

        // description: yup
        //     .string()
        //     .trim()
        //     .required('Không được để trống')
        //     .max(255, 'Tối đa 4000 ký tự'),


    });
    useEffect(()=>{
        if(!openModalEdit){
            setListOrganizationSelected([])
        }
    },[openModalEdit])
    useEffect(() => {
        if(isUpdate){
            getDetailRepositoryApi({
                id:currentRepository.id
            }).then(r=>{
                setInfo(r.data);
                if(r.data.organizationResponses){
                    setListOrganizationSelected(r.data.organizationResponses)
                }
            }).catch(e=>{

            })
            // setInfo(updateOrganization);
        }
    }, [isUpdate,currentRepository])

    const submit = (value) => {
        if(listOrganizationSelected.length==0&&(currentRepository.type=='repository'||(currentRepository.type!='repository'&&!isUpdate))){
            toast.error("Vui lòng chọn đơn vị");
            return
        }
        let ids = convertObjectsToIds(listOrganizationSelected)
        if(!isUpdate){
            createRepositoryApi(
                {
                    name:value.name,
                    code:value.code,
                    description:value.description,
                    location:value.location,
                    // parentId: selectedNodeKeys,
                    organizationIds:ids,
                    type:"repository",
                    status:value.status
                }
            ).then(r=>{
                toast.success("Thêm mới thành công")
                handleCloseModalEdit()
                setSelectedNodeKey(r.data.id)
                setIsRefresh(!isRefresh)
            })
        }
        else {
            updateRepositoryApi(
                {
                    id:currentRepository.id,
                    name:value.name,
                    code:value.code,
                    description:value.description,
                    location:value.location,
                    organizationIds:currentRepository.type=='repository'?ids:null,
                    type:currentRepository.type=='repository'?"repository":"",
                    status:value.status
                    // parentId: selectedNodeKeys
                }
            ).then(r=>{
                toast.success("Cập nhật thành công")
                handleCloseModalEdit()
                setIsRefresh(!isRefresh)

            })
        }

    }
    const convertObjectsToIds = (arr) => {
        let ids = [];
        for (let i=0;i<arr.length;i++){
            ids.push(arr[i].id)
        }
        return ids;
    }
    const getOrganizationById = (id) => {
        for (let i=0;i<listOrganization.length;i++){
            if(listOrganization[i].id == id){
                return listOrganization[i]
            }
        }
        return null;
    }
    const isExistOrganization = (id)=>{
        for (let i=0;i<listOrganizationSelected.length;i++){
            if(id == listOrganizationSelected[i].id){
                return true
            }
        }
        return false
    }
    const addOrganization = (id)=>{
        let convert = [...listOrganizationSelected];
        if(!isExistOrganization(id)){
            convert.push(getOrganizationById(id));
            setListOrganizationSelected(convert)
        }

    }
    const removeOrganization = (index) => {
      setListOrganizationSelected([...listOrganizationSelected.slice(0,index),...listOrganizationSelected.splice(index+1)])
    }
    const createRepositoryApi = (body) => {
        return apiRepository.createRepository(body)
    }
    const updateRepositoryApi = (body) => {
        return apiRepository.updateRepository(body)
    }
    const getDetailRepositoryApi = (body) => {
        return apiRepository.getDetailRepository(body)
    }
    const [filterValue, setFilterValue] = useState('');
    useEffect(()=>{
        if(!openModalEdit){
            setInfo({
                id:null,
                name: '',
                code: '',
                description: '',
                parentId: null,
                location:'',
                status:'empty'
            })
        }
    },[openModalEdit])
    const filterOptions = {
        filter: true,
        filterBy: 'label',
        filterPlaceholder: 'Tìm kiếm',
        filterMode: "strict",
        filterValue: filterValue,

    };

    return (

        <div style={{width:'400px'}}>
            <Dialog className={"modal"}  open={openModalEdit} onClose={handleCloseModalEdit}
                    fullWidth={true}
                    maxWidth={`${currentRepository.type=='repository'||(currentRepository.type!='repository'&&!isUpdate)?'md':'sm'}`}
            >

                <DialogTitle>
                    <div className={'vmp-tittle'}>
                        {
                            isUpdate?"Sửa kho":"Thêm kho"
                        }
                    </div>
                    <IconButton
                        aria-label="close"
                        onClick={handleCloseModalEdit}
                        sx={{
                            position: 'absolute',
                            right: 8,
                            top: 8,
                            color: (theme) => theme.palette.grey[500],
                        }}
                    >
                        <CloseIcon/>
                    </IconButton>
                </DialogTitle>
                <Formik
                    enableReinitialize
                    initialValues={{
                        name: info.name,
                        description: info.description,
                        code: info.code,
                        location: info.location,
                        status:info.status
                    }}
                    validationSchema={validationSchema}
                    onSubmit={
                        (values, actions) => {
                            let valueConvert = {...values};
                            submit(valueConvert)
                        }
                    }
                >
                    {props => {
                        const {
                            values,
                            touched,
                            errors,
                            handleChange,
                            setFieldValue,
                            handleSubmit
                        } = props;
                        return (
                            <Form onSubmit={handleSubmit}>
                                <Box sx={{flexGrow: 1}} className={'form-content'}>
                                    <Grid container spacing={3}>
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Tên kho<span
                                                className={'error-message'}>*</span></div>
                                            <TextField
                                                size={"small"}
                                                id='name'
                                                name='name'
                                                className={'formik-input'}
                                                // variant="standard"
                                                value={values.name}
                                                onChange={handleChange}
                                                error={touched.name && Boolean(errors.name)}
                                                helperText={touched.name && errors.name}
                                            />
                                        </Grid>

                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Mô tả</div>
                                            <TextField
                                                className={'formik-input'}
                                                // variant="standard"
                                                id='description'
                                                name='description'
                                                multiline
                                                rows={3}
                                                value={values.description}
                                                onChange={handleChange}
                                                error={touched.description && Boolean(errors.description)}
                                                helperText={touched.description && errors.description}
                                            />
                                        </Grid>
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Địa điểm</div>
                                            <TextField
                                                size={"small"}
                                                id='location'
                                                name='location'
                                                className={'formik-input'}
                                                // variant="standard"
                                                value={values.location}
                                                onChange={handleChange}
                                                error={touched.location && Boolean(errors.location)}
                                                helperText={touched.location && errors.location}
                                            />
                                        </Grid>
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Trạng thái</div>
                                            <FormControl fullWidth>
                                                <Select
                                                    id='status'
                                                    name='status'
                                                    value={values.status}
                                                    onChange={(event) => {
                                                        setFieldValue('status', event.target.value);
                                                    }}
                                                    size={"small"}>
                                                    <MenuItem value={'empty'}>Còn trống</MenuItem>
                                                    <MenuItem value={'full'}>Đã đầy</MenuItem>
                                                </Select>

                                            </FormControl>
                                        </Grid>
                                        {
                                            currentRepository.type=='repository'||(currentRepository.type!='repository'&&!isUpdate)?<Grid item xs={6} md={12}>
                                                <div className={'label-input'}>Thuộc đơn vị<span
                                                    className={'error-message'}>*</span></div>
                                                <div className={'table-organization'}>
                                                    <div style={{marginBottom:'5px'}}>
                                                        <TreeSelect
                                                            {...filterOptions} filterMode="strict"
                                                            // filterMode="strict"
                                                            value={selectedNodeKeysOrganization}
                                                            onChange={(e) => {
                                                                addOrganization(e.value);

                                                                // setSelectedNodeKeysOrganization(e.value)
                                                            }}
                                                            options={sortTreeData(listOrganizationTree)}
                                                            expandedKeys={expandedKeysOrganization}
                                                            onToggle={(e) => setExpandedKeysOrganization(e.value)}
                                                            style={{width: '250px', zIndex: '1000000 !important', overflow: 'auto'}}
                                                            className="md:w-20rem w-full"
                                                            placeholder="Chọn đơn vị"></TreeSelect>
                                                    </div>
                                                    <TableContainer className={'table-container'} style={{maxHeight:"200px"}}>
                                                        <Table stickyHeader className={"table-custom"}>
                                                            <TableHead>
                                                                <TableRow>
                                                                    <TableCell style={{width: '30px'}} align="center">STT</TableCell>
                                                                    <TableCell>Tên </TableCell>
                                                                    <TableCell style={{width: '30px'}}>Thao tác</TableCell>
                                                                </TableRow>
                                                            </TableHead>
                                                            <TableBody style={{overflowY: "auto"}}>
                                                                {/*<div className={`message-table-empty ${loading ? 'mt-30' : 'hidden'}`}>*/}
                                                                {/*    <CircularProgress size={30}></CircularProgress>*/}
                                                                {/*</div>*/}
                                                                <div
                                                                    className={`message-table-empty ${listOrganizationSelected.length === 0 ? 'mt-30' : 'hidden'}`}>
                                                                    <div>
                                                                        Vui lòng chọn đơn vị
                                                                    </div>
                                                                </div>
                                                                {
                                                                    listOrganizationSelected.map((item, index) => (
                                                                        <>
                                                                            <TableRow>
                                                                                <TableCell>
                                                                                    <div>{index + 1}</div>
                                                                                </TableCell>
                                                                                <TableCell>
                                                                                    <div>{item.name}</div>
                                                                                </TableCell>
                                                                                <TableCell style={{textAlign:'center'}}>
                                                                                    <ClearIcon onClick={() => {
                                                                                        removeOrganization(index)
                                                                                    }} color={"error"}>
                                                                                    </ClearIcon>
                                                                                </TableCell>

                                                                            </TableRow>
                                                                        </>
                                                                    ))
                                                                }

                                                            </TableBody>
                                                        </Table>
                                                    </TableContainer>
                                                </div>
                                            </Grid>:''
                                        }

                                        {/*<Grid item xs={6} md={6}>*/}
                                        {/*    <input type="file"/>*/}
                                        {/*</Grid>*/}
                                        <Grid item xs={6} md={12}>
                                            <div className={''} style={{display: "flex", justifyContent: "center"}}>
                                                <Button style={{marginRight: '10px'}} onClick={handleCloseModalEdit}
                                                        variant="outlined">Hủy</Button>
                                                <Button disabled={disable} variant="contained"
                                                        type='submit'>Lưu</Button>

                                            </div>
                                        </Grid>
                                    </Grid>
                                </Box>

                            </Form>
                        )
                    }}
                </Formik>

            </Dialog>
        </div>
    );
}